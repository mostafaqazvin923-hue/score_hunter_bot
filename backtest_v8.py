...
print("============================================================")
print("📥 دانلود داده‌ها برای موتور الیتِ نهایی (Macro + Sweep + Volume/Displacement + R:R 1:2)")
print("============================================================")
...
print("\n🚀 اجرای موتور فوق‌پیشرفته با فیلترهای حجمی و ساختاری سخت‌گیرانه...")
...
for symbol, df1h in data_1h.items():
    ...
    for i in range(250, len(df1h) - 70):
        ...
        # محاسبه اندازه بدنه کندل برای تایید جابجایی (Displacement)
        candle_body = abs(c['Close'] - c['Open'])
        
        if is_bullish_macro:
            swept_low = (
                (c['Low'] < local_low) and 
                (c['Close'] > local_low) and 
                (c['Close'] > c['Open']) and
                (c['Volume'] > c['Volume_MA'] * 1.25) and # فیلتر حجم نهادی
                (candle_body > 0.5 * c['ATR'])            # فیلتر جابجایی و قدرت کندل
            )
            ...
            swept_high = (
                (c['High'] > local_high) and 
                (c['Close'] < local_high) and 
                (c['Close'] < c['Open']) and
                (c['Volume'] > c['Volume_MA'] * 1.25) and
                (candle_body > 0.5 * c['ATR'])
            )
...
print("\n============================================================")
print("📊 گزارش نهایی موتور الیتِ نهایی (هدف: وین‌ریت ۵۰ درصد)")
print("============================================================")
if all_portfolio_trades:
    ...
    print(f"🔸 تعداد کل معاملات (بسیار فیلترشده): {total}")
