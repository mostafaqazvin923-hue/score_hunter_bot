...
# -*- coding: utf-8 -*-
"""
HUNTER-X VPBB CLEAN PORTFOLIO BACKTEST (LBANK INTEGRATED)
=========================================================
...
def signal_at(df: pd.DataFrame, i: int, cfg: Config) -> Optional[dict]:
    ...
    long_trend = r["ema20_4h"] > r["ema50_4h"] > r["ema200_4h"] and r["close"] > r["ema20_4h"] and r["rsi_4h"] > 50
    short_trend = r["ema20_4h"] < r["ema50_4h"] < r["ema200_4h"] and r["close"] < r["ema20_4h"] and r["rsi_4h"] < 50
    ...
    trend_long = r["ema20"] > r["ema50"] > r["ema200"] and r["close"] > r["ema20"] and r["rsi"] > 50 and ema_rising
    trend_short = r["ema20"] < r["ema50"] < r["ema200"] and r["close"] < r["ema20"] and r["rsi"] < 50 and ema_falling
    ...
    if r["candle_atr_ratio"] > cfg.max_candle_atr or r["ema20_dist_atr"] > cfg.max_distance_ema_atr:
        return None
    if abs(r["ema20"] - r["ema50"]) / r["close"] < cfg.flat_ema_threshold:
        return None

    poc_chop = near(r["close"], r["vp_poc"], r["atr"], 0.15)
    long_sweep = r["low"] < r["prior_swing_low"] and r["close"] > r["prior_swing_low"]
    short_sweep = r["high"] > r["prior_swing_high"] and r["close"] < r["prior_swing_high"]
    ...
    long_vp = near(r["close"], r["vp_val"], r["atr"], cfg.vp_near_atr) or near(r["close"], r["vp_poc"], r["atr"], cfg.vp_near_atr) or near(r["low"], r["bb_lower"], r["atr"], cfg.vp_near_atr)
    short_vp = near(r["close"], r["vp_vah"], r["atr"], cfg.vp_near_atr) or near(r["close"], r["vp_poc"], r["atr"], cfg.vp_near_atr) or near(r["high"], r["bb_upper"], r["atr"], cfg.vp_near_atr)

    long_score = 2 * int(long_trend and trend_long) + 2 * int(long_vp) + 2 * int(long_sweep) + int(long_bb) + int(long_rsi) + int(volume_ok) + int(r["adx"] >= cfg.adx_min)
    short_score = 2 * int(short_trend and trend_short) + 2 * int(short_vp) + 2 * int(short_sweep) + int(short_bb) + int(short_rsi) + int(volume_ok) + int(r["adx"] >= cfg.adx_min)

    if poc_chop and not long_sweep and not short_sweep:
        return None
...
# =========================
# GLOBAL PORTFOLIO SIMULATOR (LOCKED OVERLAP)
# =========================
...
def run_portfolio_backtest(dfs: Dict[str, pd.DataFrame], cfg: Config) -> pd.DataFrame:
    ...
    for ts in all_timestamps:
        ...
        # 2. Check weekly risk reset
        wk = (ts.isocalendar().year, ts.isocalendar().week)
        if current_week != wk:
            current_week = wk
            week_start_equity = equity

        # 3. Scan for new entries if concurrency limit allows
        if len(positions) < cfg.max_open_positions and consecutive_losses < cfg.max_consecutive_losses:
            for symbol, df in dfs.items():
                ...
                # Signal generated on CLOSED bar i-1, executed at bar i OPEN
                ...
                # Structural blocker filter
                buf = cfg.structure_buffer_atr * sig["atr"]
                if side == "LONG" and np.isfinite(row["prior_struct_high"]) and entry < row["prior_struct_high"] < target and (row["prior_struct_high"] - entry) > buf:
                    continue
                if side == "SHORT" and np.isfinite(row["prior_struct_low"]) and target < row["prior_struct_low"] < entry and (entry - row["prior_struct_low"]) > buf:
                    continue
...
def main():
    ...
    print("=" * 78)
    print("HUNTER-X VPBB PORTFOLIO BACKTEST (LBANK API)")
    print("=" * 78)
    print(f"Symbols: {', '.join(args.symbols)}")
    print(f"Max Concurrent Positions Locked: {cfg.max_open_positions}")
    print()
    ...
    if trades_df.empty:
        ...
        print(f"Final Equity  : ${cfg.initial_equity + net_pnl:,.2f}")
