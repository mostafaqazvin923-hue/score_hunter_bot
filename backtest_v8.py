...
CONFIG = {
    ...
    "sweep_confirm_window": 10,   # حداکثر کندل بعد از Sweep برای وقوع BOS
    "retest_window": 6,           # حداکثر کندل بعد از BOS برای Retest
    "retest_atr_mult": 0.25,      # عرض ناحیه Retest بر حسب ATR
    "sl_atr_buffer": 0.30,        # بافر ATR اضافه به Structure برای SL
    "min_sl_atr": 0.8,            # حداقل فاصله SL بر حسب ATR
    "max_sl_atr": 2.5,            # حداکثر فاصله SL بر حسب ATR
    "rr": 2.0,                    # Risk:Reward ثابت
    "vol_mult": 1.3,              # حداقل نسبت حجم کندل BOS به میانگین
    "min_atr_pct": 0.005,         # حداقل ATR/Price برای اجازه معامله (0.5%)
    "adx_min": 20,                # حداقل ADX(4H) برای رژیم روند‌دار
    "min_score": 65,              # حداقل امتیاز برای پذیرش سیگنال (از 100)
...
def main():
    ...
    print("\n✨ بک‌تست LSCS به اتمام رسید. فایل کامل معاملات: lscs_all_trades.csv")
