...
# ============================================================
# HUNTER-V74-LSP & OPTIMIZED CIRCUIT BREAKERS
# ============================================================
...
OUTPUT_DIR = "hunter_v74_optimized_output"
...
print("=" * 68)
print("HUNTER-V74 OPTIMIZED — STREAK CONTROL")
print("=" * 68)
...
def select_candidates_lsp(
    ...
    for original_rank, candidate in enumerate(candidates):
        streak = symbol_side_losses.get(
            (candidate["symbol"], candidate["side"]), 0
        )
        if streak <= 0:
            penalty = 0.0
        elif streak == 1:
        ...
        if global_loss_streak >= LSP_ACTIVATE_AT:
            penalty += GLOBAL_STREAK_PENALTY

        base_score = len(candidates) - original_rank
        adjusted_score = float(base_score) - penalty
        ranked.append((adjusted_score, original_rank, candidate))

    ranked.sort(key=lambda x: (x[0], -x[1]), reverse=True)
    return [item[2] for item in ranked[:slots]]
    ...
    for ts in all_timestamps:
        ...
        for symbol, pos in list(active_positions.items()):
            ...
            if outcome == "LOSS":
                ...
                symbol_side_losses[(symbol, pos["side"])] += 1
            else:
                global_loss_streak = 0
                symbol_side_losses[(symbol, pos["side"])] = 0
            ...
            all_trades.append(
                {
                    ...
                    "ExitPrice": exit_p,
                    "LSP_Active": int(use_lsp),
                    "LSP3_Active": int(use_lsp3),
                }
            )
        ...
        market_breadth_ratio = (
            bullish_count / total_active_syms if total_active_syms > 0 else 0.5
        )
        ...
        if use_lsp:
            selected = select_candidates_lsp(
                candidates, slots, symbol_side_losses, global_loss_streak
            )
        else:
...
if __name__ == "__main__":
    # اجرای بک‌تست با اعمال تنظیمات فایروال و محدودکننده ازدحام برای سرکوب ضررهای متوالی
    optimized_trades, equity_curve, diagnostics = run_backtest(
        processed_data,
        use_lsp=False,
        use_lsp3=True,             # فعال‌سازی مدارشکن جهت‌دار
        gate_mode="B",             # فیلتر شورت‌های بیش‌ازحد کشیده شده
        use_firewall=True,         # قفل کردن جهت پس از ثبت ضررهای متوالی
        use_single_loss_crowd=True # جلوگیری از انباشت پوزیشن‌های هم‌جهت پس از اولین ضرر
    )

    report("HUNTER-V74 OPTIMIZED (STREAK CONTROLLED)", optimized_trades, equity_curve)
