...
# ============================================================
# 1. RISK MANAGER & CONFIG (مدیریت ریسک اصلاح‌شده برای وین‌ریت بالا)
# ============================================================
...
# ============================================================
# 2. STRATEGY CORE (هسته خروج پویا بر اساس میانگین)
# ============================================================
...
class StrategyCore:
    ...
    def _check_exit(self, candle: dict, history_list) -> Optional[dict]:
        ...
        if t.direction == "long":
            ...
            # خروج به محض اینکه قیمت به خط میانگین (یا بالاتر از آن) برسد
...
# ============================================================
# 3. Z-SCORE STATISTICAL MEAN REVERSION SIGNAL (وین‌ریت بالا)
# ============================================================
...
def zscore_mean_reversion_signal(history_list):
    ...
    # محاسبه Z-Score انحراف قیمت از میانگین
    ...
    # لانگ: انحراف شدید منفی (اشباع فروش) -> بازگشت با وین‌ریت بالا به سمت میانگین
    if last_z < -2.0:
        stop_price = df["close"].iloc[-1] - (current_atr * 2.0)
        return {"direction": "long", "stop_price": stop_price, "target_ema_span": span}
    
    # شورت: انحراف شدید مثبت (اشباع خرید)
    elif last_z > 2.0:
        stop_price = df["close"].iloc[-1] + (current_atr * 2.0)
...
def run_backtest(df, signal_fn, initial_equity: float = 1000.0, risk_cfg: RiskConfig = None):
    ...
    stats = rm.stats()
    print("=" * 60)
    print("== Z-SCORE HIGH WIN-RATE BACKTEST RESULTS ==")
    print("=" * 60)
