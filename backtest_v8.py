...
print("============================================================")
print("📥 دانلود داده‌ها برای موتور ساختاری BOS (هدف قطعی: وین‌ریت ۵۰٪ و R:R 1:2)")
print("============================================================")
...
print("\n🚀 اجرای موتور هوشمند با تأییدیه شکست ساختار (BOS) برای هدف ۵۰٪...")
...
for symbol, df1h in data_1h.items():
    ...
    for i in range(250, len(df1h) - 70):
        ...
        # منطق صعودی: شکار کف + شکست ساختارِ رو به بالا (BOS) نسبت به سقف کندل قبل‌تر
        if is_bullish_macro:
            swept_low = (c['Low'] < local_low) and (c['Close'] > local_low)
            # تاییدیه BOS: کندل فعلی باید سقف کندل قبلی را بشکند تا مشخص شود خریداران قدرت را به دست گرفته‌اند
            has_bos = c['Close'] > df1h.iloc[i-1]['High']
            
            if not (swept_low and has_bos): continue
        ...
        # منطق نزولی: شکار سقف + شکست ساختارِ رو به پایین (BOS)
        elif is_bearish_macro:
            swept_high = (c['High'] > local_high) and (c['Close'] < local_high)
            has_bos = c['Close'] < df1h.iloc[i-1]['Low']
            
            if not (swept_high and has_bos): continue
            
            entry_price = c['Close']
            sl = c['High'] + (0.35 * c['ATR'])
            risk = sl - entry_price
...
print("\n============================================================")
print("📊 گزارش نهایی موتور ساختاری BOS (هدف: وین‌ریت ۵۰ درصد)")
print("============================================================")
if all_portfolio_trades:
    ...
    print(f"🎯 وین‌ریت نهایی: {win_rate:.2f}%")
