...
TARGET_RR = 2.0  # ریسک به ریوارد ۱ به ۲
...
def fetch_1year_15m_auto(symbol):
    ...
    print(f"[*] Successfully downloaded {len(candles)} candles for {symbol}")
    return candles
...
def run_backtest():
    ...
    print("==================================================")
    print(" SCORE HUNTER PRO - SMC V9.0 (OPTIMIZED TREND)")
    print("==================================================")
    ...
    for symbol in SYMBOLS:
        ...
        for i in range(200, len(candles) - 20):
            if i < skip_until:
                continue
            ...
            prev_c = sub[-3]  
            prev2_c = sub[-4] 
            current_ema = emas_200[i-1]

            lookback_slice = sub[-40:-2]
            ...
            # --- ستاپ لانگ (SMC + MSS + Trend Filter) ---
            is_sweep_low = prev_c["low"] <= recent_low
            is_bullish_mss = c["close"] > prev_c["high"] and c["close"] > c["open"]
            is_above_trend = c["close"] > current_ema

            if is_above_trend and is_sweep_low and is_bullish_mss:
                entry_price = c["close"]
                stop_loss = min(prev_c["low"], prev2_c["low"]) - (entry_price * 0.0005)
                risk_dist = entry_price - stop_loss

                if 0.001 * entry_price <= risk_dist <= 0.02 * entry_price:
                    take_profit = entry_price + (risk_dist * TARGET_RR)
                    trade_won, trade_lost = False, False
                    
                    # اجازه دادن به معامله برای حرکت تا ۴۸ کندل (۱۲ ساعت)
            ...
            # --- ستاپ شورت (SMC + MSS + Trend Filter) ---
            is_sweep_high = prev_c["high"] >= recent_high
            is_bearish_mss = c["close"] < prev_c["low"] and c["close"] < c["open"]
            is_below_trend = c["close"] < current_ema

            if not trade_taken and is_below_trend and is_sweep_high and is_bearish_mss:
                entry_price = c["close"]
                stop_loss = max(prev_c["high"], prev2_c["high"]) + (entry_price * 0.0005)
    ...
    print("\n==================================================")
    print("      AGGREGATED 1-YEAR SMC V9.0 RESULTS          ")
    print("==================================================\n")
