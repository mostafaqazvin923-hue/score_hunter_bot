...
def has_tp_space(candles, direction, entry, current_atr):
    ...
    min_structure_distance = current_atr * MIN_STRUCTURE_SPACE_ATR

    if direction == "LONG":
        resistance = max(c["high"] for c in lookback)
        if resistance - entry < min_structure_distance:
            print(f"LONG rejected: resistance too close")
            return False
        return resistance >= tp + (tp_distance * buffer)

    support = min(c["low"] for c in lookback)
    if entry - support < min_structure_distance:
        print(f"SHORT rejected: support too close")
        return False
...
def calculate_signal(candles, symbol):
    ...
    ema50_previous = ema(closes[:-1], 50)
    ...
    # CONDITION 1 - TREND
    long_trend = close > ema200 and ema50 > ema200 and ema50 > ema50_previous
    short_trend = close < ema200 and ema50 < ema200 and ema50 < ema50_previous
    ...
    bull_candle = (
        close > open_price
        and candle_range > 0
        and bull_body_ratio >= MIN_CANDLE_BODY_RATIO
        and (high - close) / candle_range <= MAX_OPPOSITE_WICK_RATIO
    )

    bear_candle = (
        close < open_price
        and candle_range > 0
        and bear_body_ratio >= MIN_CANDLE_BODY_RATIO
        and (close - low) / candle_range <= MAX_OPPOSITE_WICK_RATIO
    )
...
def main():
    ...
    print("🧭 Trend Filter: EMA50/EMA200 + EMA50 slope")
    print(f"📏 Structure Space: {MIN_STRUCTURE_SPACE_ATR:.1f} ATR")
    print(f"🕯 Candle Filter: body >= {MIN_CANDLE_BODY_RATIO:.0%}, opposite wick <= {MAX_OPPOSITE_WICK_RATIO:.0%}")
    print(
